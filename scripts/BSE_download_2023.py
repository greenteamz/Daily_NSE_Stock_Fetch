...
MASTER_CSV_FILE_PATH = os.path.join(CSV_DIR, "Master_BSE_stock_history_2023.csv")
DAILY_CSV_FILE_PATH = os.path.join(CSV_DIR, f"BSE_Stock_History23_Data_{ist_now.strftime('%Y-%m-%d')}.csv")
EXCEL_FILE_PATH = os.path.join(EXCEL_DIR, f"Stock_23{ist_now.strftime('%Y_%m')}.xlsx")
SUMMARY_CSV_FILE_PATH = os.path.join(CSV_DIR, f"Summary_BSE_Stock_23_Data_{ist_now.strftime('%Y-%m-%d')}.csv")
...
try:
    ...
    source_worksheet = spreadsheet.worksheet('symbol_bse')
...
for symbol in symbols:
    try:
        processed_count += 1
        log_message(f"BSE Processing symbol: {symbol} - {processed_count}/{len(symbols)}")
...
log_message("All BSE tasks completed successfully.")
